...
variable "secrets" {
  description = <<-EOT
    ...
    For production, do NOT include initial_value - populate secrets manually or via CI/CD.
    Example:
    {
      "openai-api-key" = {
        replication = "auto"
...
variable "create_secret_versions" {
  description = "Whether to create initial secret versions (typically false for prod - populate manually)"
  type        = bool
  default     = false
}
